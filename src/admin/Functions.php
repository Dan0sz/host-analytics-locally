...
class Functions {
	...
	private function do_update_notice() {
		...
		if ( ! $file_updated ) {
			if ( ! get_transient( self::CAOS_ADMIN_UPDATE_ERROR_MESSAGE_SHOWN ) ) {
				Notice::set_notice( sprintf( __( '%1$s doesn\'t exist or hasn\'t been updated for more than two days. Try running Update %2$s in <em>Settings > Optimize Analytics</em> to fix this. If this message returns in the next few days, consider <a href="%3$s" target="_blank">replacing WordPress\' <em>pseudo cron</em> with a real cron</a>.', 'host-analyticsjs-local' ), ucfirst( CAOS::get_current_file_key() ), CAOS::get_current_file_key(), 'https://daan.dev/docs/caos-troubleshooting/analytics-js-gtag-js-doesnt-exist/' ), 'error' );
	...
	private function do_adblock_notice() {
		...
		if ( ! get_transient( self::CAOS_ADMIN_BLOCKED_PAGES_NOTICE_SHOWN ) && $blocked_pages > 1 ) {
			Notice::set_notice( sprintf( __( "During the past 7 days, CAOS detected %1\$s pageviews on <em>%2\$s</em> with an ad blocker active. CAOS Pro's Stealth Mode <em>(starting at € 24,-)</em> bypasses Ad Blockers so you'll no longer miss out on data in Google Analytics. <a href='%3\$s'>Upgrade now</a>!", 'host-analyticsjs-local' ), number_format_i18n( get_option( self::CAOS_ADMIN_BLOCKED_PAGES_CURRENT_VALUE ) ), get_bloginfo( 'name' ), Settings::FFW_PRESS_WORDPRESS_PLUGINS_CAOS_PRO ), 'warning' );

			Notice::set_notice( sprintf( __( 'To disable these messages, disable <em>Track Ad Blockers</em> in <em>Settings > Optimize Google Analytics > <a href="%s">Extensions</a></em>.', 'host-analyticsjs-local' ), admin_url( Settings::CAOS_ADMIN_SETTINGS_EXTENSIONS_TAB_URI ) ), 'info' );
