...
class AdBlockDetect extends \WP_REST_Controller {
	...
	public function send_data( $request ) {
		...
		CAOS::debug( sprintf( __( 'User with IP %1$s has Ad Blockers %2$s.', 'host-analyticsjs-local' ), $ip, $result ) );
		...
		if ( $request->get_param( 'result' ) == 1 ) {
			$current_value = (int) get_transient( Functions::CAOS_ADMIN_BLOCKED_PAGES_CURRENT_VALUE );
			// Does not expire, but can be safely cleaned by db clean up plugins.
			set_transient( Functions::CAOS_ADMIN_BLOCKED_PAGES_CURRENT_VALUE, ++$current_value );
		}
	...
	private function get_user_ip_address() {
		...
		foreach ( self::CAOS_PRO_IP_HEADERS as $header ) {
			if ( $this->header_exists( $header ) ) {
				CAOS::debug( sprintf( __( 'HTTP header %s found.', 'host-analyticsjs-local' ), $header ) );

				$ip = $_SERVER[ $header ];

				if ( is_array( explode( ',', $ip ) ) ) {
					CAOS::debug( sprintf( __( 'Multiple IPs detected, using the first one: %s', 'host-analyticsjs-local' ), print_r( $ip, true ) ) );
