...
class Cron {
	...
	public function __construct() {
		...
		CAOS::debug( sprintf( __( 'Built file queue: %s', 'host-analyticsjs-local' ), print_r( $this->files, true ) ) );

		// Check if directory exists, otherwise create it.
		$create_dir = CAOS::create_dir_r( CAOS_LOCAL_DIR );

		if ( $create_dir ) {
			CAOS::debug( sprintf( __( '%s created successfully.', 'host-analyticsjs-local' ), CAOS_LOCAL_DIR ) );
		} else {
			CAOS::debug( sprintf( __( '%s already exists.', 'host-analyticsjs-local' ), CAOS_LOCAL_DIR ) );
		}
		...
		if ( ! wp_doing_cron() && ! empty( $downloaded_files ) ) {
			...
			Notice::set_notice( $notice . ' ' . sprintf( __( 'Would you be willing to <a href="%1$s" target="_blank">write a review</a> or <a href="%2$s" target="_blank">tweet</a> about it?', 'host-analyticsjs-local' ), $review_link, $tweet_link ), 'success', 'all', 'file_downloaded' );
		}
	}
	...
	private function build_download_queue() {
		...
		if ( $key === 'gtag' ) {
			$queue = array_merge(
				...
				[
					$key => [
						'remote' => CAOS_GTM_URL . '/gtag/js?id=' . CAOS::get( Settings::CAOS_BASIC_SETTING_TRACKING_ID ),
		...
		$dual_tracking = CAOS::get( Settings::CAOS_BASIC_SETTING_DUAL_TRACKING );

		/**
		 * If Dual Tracking is enabled, then add Gtag V4 to the download queue.
		 */
		if ( $dual_tracking === 'on' || $key === 'gtag-v4' ) {
			$tracking_id = $dual_tracking === 'on' ? CAOS::get( Settings::CAOS_BASIC_SETTING_GA4_MEASUREMENT_ID ) : CAOS::get( Settings::CAOS_BASIC_SETTING_TRACKING_ID );
	...
	private function download() {
		$i                = 0;
		$downloaded_files = [];
		$this->tweet      = sprintf( $this->tweet, CAOS::get( Settings::CAOS_ADV_SETTING_JS_FILE ) );
