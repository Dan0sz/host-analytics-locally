...
class CAOS_API_AdBlockDetect extends WP_REST_Controller {
	...
	public function send_data( $request ) {
		...
		CAOS::debug( sprintf( __( 'User with IP %1$s has Ad Blockers %2$s.', $this->plugin_text_domain ), $ip, $result ) );
		...
		$params = [
			'v'   => (string) 1,
			't'   => 'event',
			'tid' => CAOS::get( CAOS_Admin_Settings::CAOS_BASIC_SETTING_TRACKING_ID ),
			'cid' => $cid,
			'uip' => $ip,
			'ec'  => 'Tracking',
			'ea'  => 'Ad Blocker',
