...
class CAOS_FileManager {
	...
	public function download_file( $local_file, $remote_file, $file = '', $is_plugin = false ) {
		...
		if ( is_wp_error( $this->file_contents ) ) {
			CAOS::debug( sprintf( __( 'An error occurred: %1$s - %2$s', $this->plugin_text_domain ), $this->file_contents->get_error_code(), $this->file_contents->get_error_message() ) );
		...
		if ( $file_alias && file_exists( $local_dir . $file_alias ) ) {
			...
			if ( $deleted ) {
				...
				if ( $error = error_get_last() ) {
					CAOS::debug( sprintf( __( 'File %1$s could not be deleted. Something went wrong: %2$s', $this->plugin_text_domain ), $file_alias, $error['message'] ) );
				} else {
		...
		if ( $write ) {
			...
			if ( $error = error_get_last() ) {
				CAOS::debug( sprintf( __( 'File %1$s could not be saved. Something went wrong: %2$s', $this->plugin_text_domain ), $file_alias, $error['message'] ) );
			} else {
	...
	public function find_replace_in( $file, $find, $replace ) {
		CAOS::debug( sprintf( __( 'Replacing %1$s with %2$s in %3$s.', $this->plugin_text_domain ), print_r( $find, true ), print_r( $replace, true ), $file ) );
