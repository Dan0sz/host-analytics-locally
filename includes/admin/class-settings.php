        ...
        <div class="wrap">
            ...
            <?php if (CAOS_OPT_SNIPPET_TYPE != 'minimal') : ?>
                <div class="notice notice-info">
                    <p><?= sprintf(__('<strong>%s</strong> is renamed to <strong>%s</strong> and was last updated on <em>%s</em>. The next automatic update by cron is scheduled <em>%s</em>.', $this->plugin_text_domain), ucfirst(CAOS_OPT_REMOTE_JS_FILE), CAOS::get_file_alias(str_replace('.js', '', CAOS_OPT_REMOTE_JS_FILE)), $this->file_last_updated(), $this->cron_next_scheduled()); ?> <a id="caos-update-files" data-nonce="<?= wp_create_nonce(CAOS_Admin_Settings::CAOS_ADMIN_PAGE); ?>" class="button" title="<?= __('This will refresh the generated alias(es) and all files. Could be useful when running into (browser) caching issues.', $this->plugin_text_domain); ?>" href="#"><?= __('Update Now', $this->plugin_text_domain); ?></a>.</p>
                </div>
            <?php endif; ?>
